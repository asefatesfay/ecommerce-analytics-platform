...
class EcommerceDataGenerator:
    ...
    def __init__(self, 
        ...
        self.acquisition_channels = ['Organic Search', 'Paid Search', 'Social Media', 
                                   'Email Marketing', 'Direct', 'Referral', 'Affiliate']
        self.product_categories = ['Electronics', 'Clothing', 'Home & Garden', 'Books', 
                                 'Sports', 'Health & Beauty', 'Toys', 'Automotive']
    ...
    def generate_customers(self) -> pd.DataFrame:
        ...
        for i in range(self.num_customers):
            ...
            segment = np.random.choice(self.customer_segments, 
                                     p=[0.3, 0.4, 0.25, 0.05])  # Realistic distribution
            
            # Acquisition channel
            channel = np.random.choice(self.acquisition_channels,
                                     p=[0.25, 0.2, 0.15, 0.15, 0.1, 0.08, 0.07])
    ...
    def generate_products(self) -> pd.DataFrame:
        ...
        for i in range(self.num_products):
            ...
            category_price_multipliers = {
                'Electronics': 3.0, 'Automotive': 2.5, 'Home & Garden': 1.8,
                'Health & Beauty': 1.2, 'Clothing': 1.0, 'Sports': 1.5,
                'Books': 0.4, 'Toys': 0.8
            }
            ...
            product = {
                ...
                'launch_date': fake.date_between(start_date=self.start_date.date(), 
    ...
    def generate_orders(self, customers_df: pd.DataFrame, products_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        segment_behavior = {
            'Budget': {'avg_order_value': 50, 'order_frequency': 0.3, 'items_per_order': 2},
            'Standard': {'avg_order_value': 120, 'order_frequency': 0.5, 'items_per_order': 3},
            'Premium': {'avg_order_value': 300, 'order_frequency': 0.7, 'items_per_order': 5},
            'Enterprise': {'avg_order_value': 800, 'order_frequency': 0.9, 'items_per_order': 10}
        }
        
        for order_id in range(1, self.num_orders + 1):
            # Select customer (weighted by segment activity)
            segment_weights = customers_df['customer_segment'].map(
                lambda x: segment_behavior[x]['order_frequency']
            ).values
    ...
    def generate_web_analytics(self, customers_df: pd.DataFrame, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for session_id in range(1, num_sessions + 1):
            ...
            # Traffic sources
            traffic_source = np.random.choice(['organic_search', 'paid_search', 'social', 
    ...
    def generate_all_data(self) -> Dict[str, pd.DataFrame]:
        ...
        print(f"Parameters: {self.num_customers:,} customers, {self.num_products:,} products, {self.num_orders:,} orders")
